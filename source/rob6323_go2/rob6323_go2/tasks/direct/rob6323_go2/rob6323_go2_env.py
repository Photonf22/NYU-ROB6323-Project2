...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # --- NEW CODE START ---
        # Get specific body indices (CONTACT SENSOR)
        # We must look up the IDs again using the sensor object, not the robot object
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # --- NEW LOGIC IMPLEMENTATION START ---

        # 1. Penalize non-vertical orientation (projected gravity on XY plane)
        # We take the X and Y components of the gravity vector projected into the body frame.
        # If the robot is perfectly upright, X and Y are 0.
        ...
        # Simply square the Z velocity to penalize bouncing.
        rew_lin_vel_z = torch.square(self.robot.data.root_lin_vel_b[:, 2])

        # 3. Penalize high joint velocities
        # Sum the squares of velocities across all 12 joints.
        rew_dof_vel = torch.sum(torch.square(self.robot.data.joint_vel), dim=1)

        # 4. Penalize angular velocity in XY plane (roll/pitch)
        # We want to minimize rolling and pitching, so we penalize the X and Y components of angular velocity.
        rew_ang_vel_xy = torch.sum(torch.square(self.robot.data.root_ang_vel_b[:, :2]), dim=1)

        # --- NEW LOGIC IMPLEMENTATION END ---

        # 5. Foot Clearance Reward
        # Calculate a target swing trajectory (triangle/bell curve based on phase)
        # phases becomes 1.0 at the peak of the swing, 0.0 at lift-off/touch-down
        phases = 1 - torch.abs(1.0 - torch.clip((self.foot_indices * 2.0) - 1.0, 0.0, 1.0) * 2.0)
        
        # Get current foot height (Z component of world position)
        foot_height = self.foot_positions_w[:, :, 2]
        
        # Target height = 0.08m peak height + 0.02m foot radius offset
        target_height = 0.08 * phases + 0.02
        
        # Only penalize height error during SWING phase (when desired_contact_states is 0)
        rew_foot_clearance = torch.square(target_height - foot_height) * (1 - self.desired_contact_states)
        rew_feet_clearance = torch.sum(rew_foot_clearance, dim=1)

        # 6. Contact Forces Reward
        # Get the net contact forces specifically for the feet
        # Important: Use self._feet_ids_sensor here!
        ...
        # Penalize non-zero contact forces during the SWING phase
        for i in range(4):
            rew_tracking_contacts_shaped_force += - (1 - self.desired_contact_states[:, i]) * (
                1 - torch.exp(-1 * foot_forces[:, i] ** 2 / 100.)
            )
        # Scale as per reference (average over feet or just sum, reference implies specific scaling)
        # We will apply the config scale later, so we just keep the raw value here.
        # Note: The reference code divides by 4. We will do that here to normalize.
