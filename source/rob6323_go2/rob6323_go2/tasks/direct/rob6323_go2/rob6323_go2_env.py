...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, 
                 **kwargs):
        super().__init__(cfg, render_mode, **kwargs)

        # Joint position command (deviation from default joint positions)
        self._actions = torch.zeros(self.num_envs, gym.spaces.flatdim(
            self.single_action_space), device=self.device)
        ...
        self.Kp = torch.tensor(
            [cfg.Kp] * 12, device=self.device).unsqueeze(0).repeat(
                self.num_envs, 1)
        ...
        # Update Logging
        self._episode_sums = {
            key: torch.zeros(self.num_envs, dtype=torch.float,
                             device=self.device)
            for key in [
        ...
        self.last_actions = torch.zeros(self.num_envs, gym.spaces.flatdim(
            self.single_action_space), 3, dtype=torch.float,
    ...
    def _step_contact_targets(self):
        ...
        offsets = 0.
        bounds = 0.
        durations = 0.5 * torch.ones((self.num_envs,), dtype=torch.float32,
                                     device=self.device)
        self.gait_indices = torch.remainder(self.gait_indices + self.step_dt *
                                            frequencies, 1.0)

        # 6.3 Implement Rewards
        # Now, reimplement the logic from the reference code using 
        # Isaac Lab's API.
        ...
        smoothing_multiplier_FL = (
            smoothing_cdf_start(torch.remainder(foot_indices[0], 1.0)) * (
                1 - smoothing_cdf_start(
                    torch.remainder(
                        foot_indices[0], 1.0) - 0.5)) + smoothing_cdf_start(
                            torch.remainder(foot_indices[0], 1.0) - 1) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[0], 1.0) - 0.5 - 1)))
        smoothing_multiplier_FR = (
            smoothing_cdf_start(torch.remainder(foot_indices[1], 1.0)) * (
                1 - smoothing_cdf_start(torch.remainder(foot_indices[1], 1.0)
                                        - 0.5)) + smoothing_cdf_start(
                                            torch.remainder(foot_indices[1], 
                                                            1.0) - 1) * (
                                            1 - smoothing_cdf_start(
                                                torch.remainder(foot_indices[1]
                                                                , 1.0) - 0.5 - 1)))
        smoothing_multiplier_RL = (
            smoothing_cdf_start(torch.remainder(foot_indices[2], 1.0)) * (
                1 - smoothing_cdf_start(torch.remainder(foot_indices[2], 1.0)
                                        - 0.5)) + smoothing_cdf_start(
        ...
        smoothing_multiplier_RR = (
            smoothing_cdf_start(torch.remainder(foot_indices[3], 1.0)) * (
                1 - smoothing_cdf_start(torch.remainder(foot_indices[3], 1.0) -
                                        0.5)) + smoothing_cdf_start(
                                            torch.remainder(foot_indices[3],
                                                            1.0) - 1) * (
    ...
    # We calculate the error between where the foot IS and where the 
    # Raibert Heuristic says it SHOULD be.
    def _reward_raibert_heuristic(self):
        ...
        err_raibert_heuristic = torch.abs(desired_footsteps_body_frame -
                                          footsteps_in_body_frame[:, :, 0:2])
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        lin_vel_error = torch.sum(torch.square(self._commands[:, :2] -
        ...
        rew_action_rate = torch.sum(torch.square(
            self._actions - self.last_actions[:, :, 0]), dim=1) * (
                self.cfg.action_scale ** 2)
        # Second derivative (Current - 2*Last + 2ndLast)
        rew_action_rate += torch.sum(torch.square(
            self._actions - 2 * self.last_actions[:, :, 0] +
            self.last_actions[:, :, 1]), dim=1) * (self.cfg.action_scale ** 2)
        ...
        rew_feet_clearance = torch.sum(swing_prob * (
            foot_height - target_clearance).pow(2), dim=1)  # (N,)

        # 5.2 Implement Reward Terms
        # 1. Penalize non-vertical orientation (projected gravity on XY plane)
        # Hint: We want the robot to stay upright, so gravity should only
        # project onto Z.
        # Calculate the sum of squares of the X and Y components of
        # projected_gravity_b.
        rew_orient = torch.sum(
            self.robot.data.projected_gravity_b[:, :2] ** 2, dim=1)
        ...
        # Hint: Sum the squares of the X and Y components 
        # of the base angular velocity.
    ...
    # self.contact_forces -> Use self._contact_sensor.data.net_forces_w. 
    # Important: Index this using self._feet_ids_sensor.
    def _reward_tracking_contacts_shaped_force(self) -> torch.Tensor:
        ...
        # Both FL and RR in contact
        pair1_sync = in_contact[:, 0] * in_contact[:, 3]
        # Both FR and RL in contact
        pair2_sync = in_contact[:, 1] * in_contact[:, 2]
        # Penalize when non-diagonal feet are in contact (pacing or bounding)
        # FL and FR (front pair)
        pair1_opposite = in_contact[:, 0] * in_contact[:, 1]
        # RL and RR (rear pair)
        pair2_opposite = in_contact[:, 2] * in_contact[:, 3]
        # Combine rewards
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        cstr_termination_contacts = torch.any(torch.max(torch.norm(
            net_contact_forces[:, :, self._base_id], dim=-1), dim=1)[0] > 1.0,
            dim=1)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if len(env_ids) == self.num_envs:
            # Spread out the resets to avoid spikes in training when many 
            # environments reset at a similar time
    ...
    def _debug_vis_callback(self, event):
        ...
        # note: this is needed in-case the robot is de-initialized.
    ...
    def _resolve_xy_velocity_to_arrow(
            self, xy_velocity: torch.Tensor) -> tuple[torch.Tensor,
                                                      torch.Tensor]:
        """Converts the XY base velocity command
        to arrow direction rotation."""
